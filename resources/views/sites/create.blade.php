...
@section('content')
    <div class="mb-6">
        ...
        <h1 class="text-2xl font-bold text-base-content mt-2">Create Site</h1>
    </div>

    <div class="card bg-base-100 shadow-sm max-w-2xl">
        <div class="card-body">
            <form method="POST" action="{{ route('sites.store') }}" x-data="pageManager()">
                @csrf
                ...
                <div class="form-control mb-4">
                    ...
                    <textarea name="description" id="description" rows="3" maxlength="500" class="textarea textarea-bordered w-full">{{ old('description') }}</textarea>
                    @error('description')
                        <label class="label">
                            <span class="label-text-alt text-error">{{ $message }}</span>
                        </label>
                    ...
                    <select name="responsible_person_id" id="responsible_person_id" class="select select-bordered w-full"
                        ...
                        @foreach ($staffMembers as $staff)
                            <option value="{{ $staff->id }}"
                                {{ old('responsible_person_id') == $staff->id ? 'selected' : '' }}>
